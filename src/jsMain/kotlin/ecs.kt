...
class World<out Ctx>(
    ...
    private val groupsMap = mutableMapOf<String, MutableSet<Int>>()

    val tags: Map<String, Int>
        get() = tagsMap

    val groups: Map<String, Collection<Int>>
        get() = groupsMap
    ...
    fun addToGroup(group: String, id: Int) {
        delay {
            groupsMap[group] = groupsMap.getOrElse(group) { mutableSetOf() }.run {
                add(id)
                this
            }
        }
    ...
    fun removeFromGroups(id: Int) {
        delay {
            groupsMap.forEach { e -> e.value.remove(id) }
        }
    }
